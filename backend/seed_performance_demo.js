...
// Deterministic randomness ------------------------------------------------ A fixed seed so two.
...
// Task titles by department ---------------------------------------------- Written.
const TITLES = {
    Engineering: [
...
const CAST = [
    ...
    {
        // Slipping: busy early, quiet since.
    ...
    {
        // Marketing is a two-person department and Mehedi does most of it.
...
const rampAt = (trend, i, span) => {
    const t = i / span
    // A gentle ramp on purpose.
    if (trend === 'rising') return 0.72 + 0.56 * t
    if (trend === 'falling') return 1.28 - 0.56 * t
    return 1
}

// Place each completion into a week so that the *weighted hours* per week follow the person's.
const assignWeeksByHours = (weights, weeks, trend) => {
    ...
    for (const { i, w } of order) {
        // Every week starts with an identical deficit.
...
const buildFor = (person, employee, objectives, to, titlePool) => {
    ...
    // Decide what each completion *is* before deciding when it happened.
    ...
    for (let i = 0; i < person.done; i += 1) {
        ...
        // Logged hours drift away from the estimate by however accurate this person is meant to be.
...
const seed = async () => {
    ...
    // Two critical tasks with nobody on them.
    docs.push({
